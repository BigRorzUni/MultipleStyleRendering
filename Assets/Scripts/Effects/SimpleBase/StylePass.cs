...
[System.Serializable]
public class StylePass : EffectPass
{
    ...
    static readonly int StyleBitID = Shader.PropertyToID("_StyleBit");
    ...
    public StylePass(Shader shader, StyleBits.ImageSpaceEffect requiredBit, string name) : base(shader, name, requiredBit)
    {
        passname = name;
    }
    ...
    protected override void RunFullscreen(RenderGraph renderGraph, UniversalResourceData frameData, UniversalCameraData cameraData, NprFrameData nprFrameData)
    {
        using (var builder = renderGraph.AddRasterRenderPass($"Fullscreen {passName} Pass", out PassData passData))
        {
            ...
            builder.SetRenderFunc(static (PassData data, RasterGraphContext ctx) =>
            {
                ...
                data.mat.SetInt(StyleBitID, data.requiredBit);
    ...
    protected override void RunCpu(RenderGraph renderGraph, UniversalResourceData frameData, UniversalCameraData cameraData, NprFrameData nprFrameData)
    {
        ...
        foreach (var bbox in nprFrameData.bboxes)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass($"BBox {passName} ({bbox.box})", out PassData passData))
            {
