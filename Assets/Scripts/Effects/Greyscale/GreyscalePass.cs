...
[System.Serializable]
public class GreyscalePass : EffectPass
{
    ...
    static readonly int GreyscaleBitID = Shader.PropertyToID("_GreyscaleBit");
    ...
    public GreyscalePass(Shader shader, StyleBits.ImageSpaceEffect requiredBit) : base(shader, "GreyscalePass", requiredBit)
    {
    }
    ...
    {
        using (var builder = renderGraph.AddRasterRenderPass("Fullscreen Greyscale Pass", out PassData passData))
        {
            ...
            builder.SetRenderFunc(static (PassData data, RasterGraphContext ctx) =>
            {
                ...
                data.mat.SetInt(GreyscaleBitID, data.requiredBit);
    ...
    protected override void RunCpu(RenderGraph renderGraph, UniversalResourceData frameData, UniversalCameraData cameraData, NprFrameData nprFrameData)
    {
        ...
        foreach (var bbox in nprFrameData.bboxes)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass($"BBox Greyscale ({bbox.box})", out PassData passData))
            {
    ...
    protected override void RunGpu(RenderGraph renderGraph, UniversalResourceData frameData, UniversalCameraData cameraData, NprFrameData nprFrameData)
    {
        ...
        using (var builder = renderGraph.AddRasterRenderPass("Batched Greyscale Pass (GPU)", out PassData passData))
        {
            passData.src = nprFrameData.sourceTexture;
            passData.ids = nprFrameData.idTexture;
            passData.mat = _mat;
