...
public static class CsvWriter
{
    ...
    public static void EnsureDirectoryExists(string directoryPath)
    {
        ...
        {
            Debug.LogError("directoryPath for writing csvs was null or empty.");
            return;
        }
    ...
    public static void WriteFrameTimings(string path, double[] cpuTimings, double[] gpuTimings)
    {
    ...
    public static void AppendSummaryRow(string path, NprTestCase test, int value, NprRenderMode renderMode, int curN, int curK, int curS, double[] cpuTimings, double[] gpuTimings)
    {
        ...
        if (!fileExists)
        {
            sw.WriteLine("test_name,scene,variable,value,render_mode,N,K,styles_per_object," + "mean_cpu_ms,median_cpu_ms,p95_cpu_ms,p99_cpu_ms,max_cpu_ms,std_cpu_ms," + "mean_gpu_ms,median_gpu_ms,p95_gpu_ms,p99_gpu_ms,max_gpu_ms,std_gpu_ms");
        }
    ...
    public static void AppendPassSummaryRows(string path, NprTestCase test, int value, NprRenderMode renderMode, int curN, int curK, int curS)
    {
        if (string.IsNullOrWhiteSpace(path))
        {
            Debug.LogError("Path was null or empty when writing to csv");
            return;
        }

        if (test == null)
        {
            Debug.LogError("test was null when writing to csv");
            return;
        }
        ...
        if (!fileExists)
        {
            sw.WriteLine("test_name,scene,variable,value,render_mode,N,K,styles_per_object,pass_name," + "mean_cpu_ms,median_cpu_ms,p95_cpu_ms,p99_cpu_ms,max_cpu_ms,std_cpu_ms," + "mean_gpu_ms,median_gpu_ms,p95_gpu_ms,p99_gpu_ms,max_gpu_ms,std_gpu_ms");
        }
    }
